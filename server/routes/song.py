...
router = APIRouter()
cloudinary.config( 
    cloud_name = "dijpf0fmq", 
    api_key = "222675696629784", 
    api_secret = "JZ3BFSeIZRJcVcbCmGUyD01igXA", # Click 'View Credentials' below to copy your API secret
    secure=True
)
...
def favorite_song( song: FavoriteSong,db: Session = Depends(get_db),auth_details = Depends(auth_middleware), ):
    ...
    #If favorited then unfavorite the song 
    #Not favorited then favorite the song
    if fav_song:
        db.delete(fav_song)
        db.commit()
